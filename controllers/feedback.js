...
const newsController = {
  ...
  selectAll: async (req, resp, next) => {
    ...
    const [news, count] = await findAndCount(db.Feedback, { ...userQueryOption, ...queryOption}, {
      skip: offset,
      limit: limit,
      sort: {
        createdAt: -1,
  ...
  process: async (req, resp, next) => {
    let piece = await db.Feedback.findById(req.params.id);
    if (!piece) {
      resp.failed('没找到反馈！');
    } else {
